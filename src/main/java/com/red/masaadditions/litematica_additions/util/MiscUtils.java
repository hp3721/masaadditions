...
public class MiscUtils {
    static final Item[] IGNORED_ITEMS_VALUES = new Item[] { Items.PISTON, Items.STICKY_PISTON, Items.WATER_BUCKET, Items.POTION, Items.LAVA_BUCKET, Items.FLINT_AND_STEEL, Items.FIRE_CHARGE };
    ...
    public static boolean checkHeldItem(ItemStack heldItemStack, BlockState stateSchematic) {
        ...
        } else if (stateSchematic.getBlock() == Blocks.CAULDRON) {
            match = heldItem == Items.POTION && PotionUtil.getPotion(heldItemStack) == Potions.WATER || (stateSchematic.get(CauldronBlock.LEVEL) == 3 && heldItem == Items.WATER_BUCKET);
        } else if (stateSchematic.getFluidState().getFluid() == Fluids.LAVA || stateSchematic.getFluidState().getFluid() == Fluids.FLOWING_LAVA) {
